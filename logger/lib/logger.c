...
int print_log(char *name){
	...
	const char *sql_program = "SELECT level, program, content, time FROM LOG WHERE program = ?";
	const char *sql_all = "SELECT level, program, content, time FROM LOG";
	sqlite3_stmt *stmt;
	int err;
	
	if(name){
		err = sqlite3_prepare_v2(db, sql_program, -1, &stmt, NULL);
	}
	else {
		err = sqlite3_prepare_v2(db, sql_all, -1, &stmt, NULL);
	}
